...
namespace BasicLib.Forms.Table{
	...
	public class TableView : CompoundScrollableControl{
		...
		public TableView(string name){
			...
			ResizeRedraw = true;
			InitContextMenu();
			columnViewToolTip.ToolTipIcon = ToolTipIcon.Info;
			...
			if (columnWidthSums != null && columnWidthSums.Length > 0) {
				...
				if (startInd >= 0) {
					for (int i = startInd; i <= endInd; i++) {
						...
						string[] q = GetStringHeader(g, i, w, textFont);
						for (int j = 0; j < q.Length; j++) {
							g.DrawString(q[j], textFont, Brushes.Black, x1 + 3, 4 + 11 * j);
						}
					}
					if (sortCol != -1 && sortState != SortState.Unsorted) {
						int x1 = columnWidthSums[sortCol] - x - 11;
						if (x1 >= -15 && x1 <= width) {
							g.DrawImage(sortState == SortState.Increasing ? Resources.arrowDown : Resources.arrowUp, x1, 6, 9, 13);
						}
					}
					if (helpCol != -1) {
						int x1 = (helpCol == 0 ? 0 : columnWidthSums[helpCol - 1]) - x + 5;
						if (x1 >= -15 && x1 <= width) {
							g.DrawImage(Resources.question1, x1, 7, 10, 10);
						}
					}
					if (model != null && model.AnnotationRowsCount > 0) {
						for (int i = startInd; i <= endInd; i++) {
							...
							for (int k = 0; k < model.AnnotationRowsCount; k++) {
								...
								if (s != null) {
									g.DrawString("" + GetStringValue(g, s, x2 - x1 - 2, textFont), textFont, Brushes.Black, x1 + 3, y1 + 3);
								}
							}
						}
					}
		...
		protected internal override sealed void OnPaintCornerView(IGraphics g) {
			...
			if (matrixHelp) {
				g.DrawImage(Resources.question1, 7, 7, 10, 10);
			}
			if (model != null && model.AnnotationRowsCount > 0) {
				for (int i = 0; i < model.AnnotationRowsCount; i++) {
					...
					if (s != null) {
						g.DrawString("" + GetStringValue(g, s, RowHeaderWidth - 6, textFont), textFont, Brushes.Black, 3, y1 + 3);
					}
				}
			}
		}
